...
class EncryptorApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Encrypt File (In-Place)")
        ...
        tk.Button(btn_frame, text="Encrypt", width=14, command=self.encrypt_action).pack(side='left', padx=8)
        ...
        tk.Label(self, text="⚠️ Warning: The file itself will be overwritten.\n"
                            "Without the password, you cannot recover its contents.",
                 fg='red', justify='left').pack(padx=10, anchor='w')
    ...
    def encrypt_action(self):
        ...
        if not os.path.isfile(path):
            messagebox.showerror("Error", "Please select an existing file to encrypt.")
            return
        if not pw:
            messagebox.showerror("Error", "Please provide a password.")
            return
        if not messagebox.askokcancel("Confirm", f"This will encrypt and overwrite the file:\n{path}\n\nContinue?"):
            return
        ...
        try:
            ...
            encrypted_data = salt + fernet.encrypt(original_data)
            with open(path, "wb") as f:
                f.write(encrypted_data)

            # Custom message after encryption
            self.show_encrypted_message()
    ...
    def show_encrypted_message(self):
        ...
        tk.Label(win, text="🔒 YOUR FILE HAS BEEN ENCRYPTED 🔒", font=("Arial", 14, "bold"), fg="red").pack(pady=30)
        tk.Label(win, text="Keep your password safe.\nYou’ll need it to decrypt the file.", fg="black").pack()
